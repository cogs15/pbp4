...
update 16ncaa_merged_pbp
 ...
 update 16ncaa_merged_pbp t1, (select game_id, event_id, adj_time, play_text from 16ncaa_merged_pbp) t2
 ...
 where poss_start=1;
  ...
  update 16ncaa_merged_pbp t1, (select event_id, game_id, poss_end from 16ncaa_merged_pbp) t2
  set t1.poss_time= 0
  ...
  update 16ncaa_merged_pbp t1, (select game_id, event_id, team_id, poss_time, poss_start, poss_end, game_time from 16ncaa_merged_pbp) t2
set t1.time_elapsed = (t1.game_time - t2.game_time)
where t1.game_id=t2.game_id and t1.event_id = t2.event_id+1;

UPDATE
    16ncaa_merged_pbp AS p
  JOIN
    ( SELECT
          p1.event_id, p1.game_id
        , count(*) AS cnt
      FROM
          16ncaa_merged_pbp AS p1
        JOIN
          16ncaa_merged_pbp AS p2
...
SET
  ...
  update 16ncaa_merged_pbp t1, (select game_id, event_id, poss_number, team_id, poss_time, poss_start, poss_end, game_time from 16ncaa_merged_pbp order by game_id, event_id) t2
...
where t1.game_id=t2.game_id and t1.event_id = t2.event_id+1 and t1.poss_number=t2.poss_number;
    ...
    UPDATE
      ...
      JOIN
        ( SELECT
              p1.event_id, p1.game_id, p1.poss_number
            , sum(p1.time_elapsed) AS cnt
          FROM
              16ncaa_merged_pbp AS p1
            JOIN
              16ncaa_merged_pbp AS p2
                ON p1.game_id = p2.game_id and
                p1.event_id < p2.event_id and
                         p1.poss_number=p2.poss_number
          GROUP BY
              p1.game_id, p1.event_id
    ...
    set penalty_clock= (split_part(penalty_length,':',1) + split_part(penalty_length,':',2)/60)
            ...
            update 16ncaa_merged_pbp t1, (select game_id, event_id, penalty_clock, emo_team from 16ncaa_merged_pbp order by event_id) t2
          ...
          emo_team = team_name
             ...
             set clear_good = 1
              ...
              set clear_fail = 1
               ...
               update 16ncaa_merged_pbp t1, (select game_id, event_id, play_text, team_id, clear_fail from 16ncaa_merged_pbp order by event_id) t2
             ...
             update 16ncaa_merged_pbp t1, (select game_id, event_id, play_text, clear_good from 16ncaa_merged_pbp order by event_id) t2
            set t1.unsettled=1
