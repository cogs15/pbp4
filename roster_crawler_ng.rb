...
team_pages.each_with_index do |page|
  ...
  #  sleep 10
  #  open("page.html","w"){|f| f.puts doc.html}
  begin
    doc = Nokogiri::HTML(open(game_url))
  rescue OpenURI::HTTPError => e
    puts "Can't access #{ game_url }"
    puts e.message
    puts
    next
  end
...
if doc.table(:id => 'sortable_roster').exists?
...
elsif doc.div(:id => 'mainbody').exists?
...
#elsif doc.xpath("//*[@id='mainbody']//table/tbody/tr[1]/td").exists?
...
#    t_id = 'mainbody'
  ...
  elsif doc.table(:id => 'ctl00_cplhMainContent_dgrdRoster').exists?
...
elsif doc.table(:id => 'main-content').exists?
